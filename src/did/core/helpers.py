...
def fallback_scan(
    ...
    # Conservative name fallback for multi-token capitalized names. Statistical
    # NER occasionally misses long names containing particles (for example
    # "Stine Louise Eising von Christierson"). Existing shorter detections take
    # precedence so sentence-leading words are not unnecessarily absorbed.
    name_pattern = re.compile(
        r"\b[A-ZÆØÅ][A-Za-zÆØÅæøåÀ-ÖØ-öø-ÿ'’-]+"
        r"(?:\s+(?:(?:von|van|de|del|der|af)\s+)?"
        r"[A-ZÆØÅ][A-Za-zÆØÅæøåÀ-ÖØ-öø-ÿ'’-]+){1,4}\b"
    )
    existing_people = list(all_entities["person"])
    for match in name_pattern.finditer(text):
        candidate = match.group()
        if any(person.casefold() in candidate.casefold() for person in existing_people):
            continue
        if candidate not in all_entities["person"]:
            all_entities["person"].append(candidate)
