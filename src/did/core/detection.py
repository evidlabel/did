...
def detect_entities(anonymizer, texts: list):
    ...
    for text in texts:
        ...
        for result in selected_results:
            ...
            if ent_type in TYPE_MAPPING and entity_text:
                ...
                entity_scores[mapped][entity_text] = max(
                    float(result.score),
                    entity_scores[mapped].get(entity_text, 0.0),
                )
    ...
    for cat in CATEGORIES:
        ...
        # Include deterministic fallback detections in the public counts.
        anonymizer.counts[f"{cat}_found"] = len(items)
        if cat == "person":
            grouped = find_name_variants(items)
        elif cat in ["organization", "email_address", "location", "url"]:
        ...
        for variants in grouped:
            ...
            scores = [
                entity_scores[cat][variant]
                for variant in variants
                if variant in entity_scores[cat]
            ]
            getattr(anonymizer.entities, cat).append(
                Entity(
                    id=f"{ent_type_upper}_{count}",
                    variants=variants,
                    confidence=round(max(scores), 4) if scores else None,
                )
            )
            count += 1
