...
def anonymize_file(input_path: Path, anonymizer: Anonymizer, output_path: Path) -> dict:
    ...
    if input_path.suffix.lower() in [".md", ".txt", ".tex", ".pdf", ".docx"]:
...
def export_to_typst(
    ...
    """
    if input_path.suffix.lower() not in [".md", ".txt", ".pdf", ".docx"]:
        raise ValueError(
            "Typst export currently supported only for .md, .txt, .pdf, and .docx files."
        )
    ...
    with open(main_path, "w", encoding="utf-8") as f:
        ...
        if input_path.suffix.lower() in [".md", ".pdf"]:
